...
async function getWithRetries(url, tries = 5) {
  ...
  for (let i = 1; i <= tries; i++) {
    try {
      return await axios.get(url); // axios instance is tuned in lib/http.js
    } catch (err) {
      lastErr = err;
      if (i < tries) {
        const wait = waits[i - 1] || 9000;
        console.warn(`[master] try ${i}/${tries} failed → ${err?.message}. retry in ${wait}ms`);
        await sleep(wait);
      }
    }
  }
...
async function fetchAndScrape(url) {
  const { data: html } = await getWithRetries(url, 5);
